    ...
    <aside className="w-64 bg-gray-800 text-white h-screen flex flex-col">
      <div className="p-5 flex-grow overflow-y-auto">
        <a className="block mb-8">
          <h2 className="text-2xl font-bold text-blue-400">tâche.</h2>
        </a>
        <nav>
          <ul className="space-y-3">
            <SidebarItem icon={<DashboardIcon />} label="Dashboard" />
            <SidebarItem icon={<StatisticsIcon />} label="Statistics" />
...
const SidebarItem = ({ icon, label }: { icon: React.ReactNode; label: string }) => (
  <li>
    <a className="flex items-center space-x-3 text-gray-300 hover:text-white hover:bg-gray-700 rounded-lg p-2 transition-colors duration-200">
      {icon}
      <span>{label}</span>
    </a>
  </li>
